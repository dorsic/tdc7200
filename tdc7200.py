...
""" Based on 
https://raw.githubusercontent.com/HowardALandman/QTD/master/src/tdc7201/tdc7201/__init__.py
"""

""" Driver for the Texas Instruments tdc7200 time-to-digital converter chip.
    Web page: http://www.ti.com/product/tdc7200

    This code assumes that you are working with the TDC7200 chip.
...
__version__ = '0.20.1'	# Use SemVer style version numbers
...
class TDC7200():
    """Interface to a Texas Instruments tdc7200 time-to-digital converter chip."""
    ...
    def __init__(self):
        ...
        self.chip_select = 1
        #print("SPI interface started to tdc7200.")
    ...
    def initGPIO(self,
                 ...
                 trig1=40,	# GPIO  4 = header pin  7
                 int1=38,	# GPIO 26 = header pin 37
                 start=29,	# GPIO 24 = header pin 18
        ...
        """Initialize all the non-SPI GPIO pins.
           ...
           NOTE that chip CS1 is wired to Pi CE0!

           ENABLE high turns chip on, ENABLE low forces complete reset.

           The TRIG signal indicates that the converter is ready to start.
           ...
           The INT signal going low indicates that measurement has stopped.
        ...
        print("Initializing TDC7200 driver.")
    ...
    def off(self):
        """Close SPI, turn TDC7200 off, and wait for reset to take effect."""
        print("Turning off tdc7200.")
    ...
    def on(self):
        """Turn TDC7200 on and wait for it to stabilize."""
        now = time.time()
        print("TDC7200 enabled at", now)
        # Turn on chip enable.
    ...
    def configure(self,
           clk_freq=8000000,	# (external) clock frequency
        ...
        if retain_state:
            ...
            if (clk_freq > 16000000):
                print("Clock frequency greated then 16 MHz maximum.")
            elif (clk_freq < 4000000):
                print("Clock frequency less then recommended 4 MHz minimum.")

            self.clockFrequency = clk_freq
            self.clockPeriod = 1.0 / self.clockFrequency
        ...
        if cf1_read == 0 and cf1_state != 0:
            print("Are you sure the TDC7200 is connected to the Pi's SPI interface?")
            self.exit()
        ...
        if self.trig1:
            ...
            if trig_falling:
                if GPIO.input(self.trig1):	# Don't wait for a falling edge if it's already low!
                    channel = GPIO.wait_for_edge(self.trig1, GPIO.FALLING, timeout=10000)
                    if channel is None:
                ...
                if not GPIO.input(self.trig1):	# Don't wait for a rising edge if it's already high!
                    channel = GPIO.wait_for_edge(self.trig1, GPIO.RISING, timeout=10000)
                    if channel is None:
...
#            return 9
        ...
        if GPIO.input(self.int1):	# Don't wait for an edge if it's already low!
            ...
            channel = GPIO.wait_for_edge(self.int1, GPIO.FALLING, timeout=2000)
            if channel is None:
        ...
        self.read_regs24()
        self.reg1[self.INT_STATUS] = self.read8(self.INT_STATUS)
        #print("Register values: {}".format(self.reg1))
    ...
    def read_data(self):
        time_regs = [self.TIME1,self.TIME2,self.TIME3,self.TIME4,self.TIME5, self.TIME6]
        clock_regs = [self.CLOCK_COUNT1,self.CLOCK_COUNT2,self.CLOCK_COUNT3,self.CLOCK_COUNT4,self.CLOCK_COUNT5]
        time_counts = []
        clock_counts = []
        times = []
        calibrations = [self.reg1[self.CALIBRATION1], self.reg1[self.CALIBRATION2]]
        if (calibrations[1]-calibrations[0]) == 0:
            return None
        normLSB = self.clockPeriod/( (calibrations[1]-calibrations[0])/(self.cal_pers-1) )

        for tm in range(self.num_stops):
            time_count = self.reg1[time_regs[tm]]
            timelookup_count = self.reg1[time_regs[tm+1]]
            clock_count = self.reg1[clock_regs[tm]]
            if self.meas_mode == 1:
                time = (time_count) * normLSB
            elif self.meas_mode == 2:
                time = (time_count-timelookup_count) * normLSB + clock_count * self.clockPeriod
            else:
                time = None
            time_counts.append(time_count)
            clock_counts.append(clock_count)
            times.append(time)
        if self.reg1[self.INT_STATUS] & self._IM_CLOCK_OVF or self.reg1[self.INT_STATUS] & self._IM_COARSE_OVF:
            times = self.num_stops*[None]

        return (times, normLSB, time_counts, clock_counts, calibrations, self.reg1[self.INT_STATUS])
